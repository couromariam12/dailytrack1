...
describe("collaborator UI states", () => {
  ...
  it("keeps pull requests distinct from issues in their respective lists", () => {
    ...
    const issueMarkup = renderToStaticMarkup(createElement(IssueList, { items: ([issue]), currentLogin: null }));
    ...
    const pullMarkup = renderToStaticMarkup(createElement(PullRequestList, { items: ([pull]), currentLogin: null }));
  ...
  it("renders only Gitea-provided activity links with safe target attributes", () => {
    ...
    const markup = [
      renderToStaticMarkup(createElement(IssueList, { items: ([issue]), currentLogin: null })),
      renderToStaticMarkup(createElement(PullRequestList, { items: ([pull]), currentLogin: null })),
      renderToStaticMarkup(createElement(CommitList, { items: ([commit]), currentLogin: null })),
      renderToStaticMarkup(createElement(ReviewList, { items: ([review]) })),
    ].join(" ");
  ...
  it("does not render links when Gitea did not provide URLs", () => {
    ...
    const markup = renderToStaticMarkup(createElement(IssueList, { items: ([issue]), currentLogin: null })) + renderToStaticMarkup(createElement(ReviewList, { items: ([review]) }));
